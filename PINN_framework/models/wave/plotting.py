...
def get_plot_data(geometry_settings, forward, params, grid):
    ...
    tlim = geometry_settings["domain"]["rectangle"]["tlim"]

    X, Y, plotpoints = get_plot_variables(xlim, tlim, grid=grid)

    # Prediction
    Z = netmap(forward)(params, plotpoints).reshape(X.shape)
    Z_true = analytic.get_true_vals({"data": plotpoints})["data"].reshape(X.shape)
